...
#define USE_GAME_ROTATION 1 // Set to 1 if you want to use Game Rotation Vector Report
#define REPORT_INTERVAL 40 //Report interval in ms
#define CALIBRATION 0 //Set to 1 if you want to enable gyro background calibration (not enable by default)
...
//Add by Math: BNO08x instance
#include "BNO08x_AOG.h"
BNO080 myIMU;
boolean useBNO08xRoll = USE_BNO08X_ROLL;
boolean useBNO08xHeading = USE_BNO08X_HEADING;
boolean bno08xEnable = useBNO08xHeading | useBNO08xRoll;
boolean useGameRotationVectorReport = USE_GAME_ROTATION;
int bno08xCalibration = CALIBRATION;
float bno08xYaw = 0;
double bno08xRoll = 0;
...
{ 
  ...
  // BNO055 init
  if (aogSettings.BNOInstalled && useBNO08xHeading == 0) //Add by Math: if BNO08x is choose for heading, BNO055 is not init
  { 
    IMU.init();  
    IMU.setExtCrystalUse(true);   //use external 32K crystal
  }  
  	
  if (aogSettings.InclinometerInstalled == 2 && useBNO08xRoll == 0) //Add by Math: if BNO08x is choose for roll, MMA is not init
  { 
  ...
  else if (aogSettings.InclinometerInstalled == 3 && useBNO08xRoll == 0) //Add by Math: if BNO08x is choose for roll, MMA is not init
  { 
      // MMA8452 (1) Inclinometer
...
//Add by Math: BNO08x init and configuration
	if (bno08xEnable == 1)
	{
		//BNO085 init
		if (myIMU.begin(BNO08x_ADRESS) == false)
		{
		...
		if (useGameRotationVectorReport == 1)
		{
			myIMU.enableGameRotationVector(REPORT_INTERVAL); //Send data update every REPORT_INTERVAL in ms for BNO085
		}
		else
		{	
			myIMU.enableRotationVector(REPORT_INTERVAL); //Send data update every REPORT_INTERVAL in ms for BNO085
		}
		...
		if (myIMU.getFeatureResponseAvailable() == true)
		{
			...
			{
				if (myIMU.checkReportEnable(SENSOR_REPORTID_ROTATION_VECTOR,REPORT_INTERVAL) == false) myIMU.printGetFeatureResponse();
			}
		}
		else Serial.println(F("BNO08x init fails!!"));
		
		//Enable Gyro background calibration if wanted and print calibration status
		if (bno08xCalibration == 1)
		{
			myIMU.calibrateAll();
			if (myIMU.printMECalibrationRespond() == false) Serial.println(F("Configure ME Calibration Command Respond Report failled to retrieve."));
		}
		//Disable background calibration if wanted and print calibration status
		else if (bno08xCalibration == 2)
		{
...
void loop()
{
    ...
    if (aogSettings.BNOInstalled && useBNO08xHeading == 0) //Add by Math: if BNO08x is use for heading, BNO055 heading is not read
        IMU.readIMU();
    
    //DOGS2 inclinometer
    if (aogSettings.InclinometerInstalled == 1 && useBNO08xRoll == 0) //Add by Math: if BNO08x is use for roll, DOGS2 is not read
    {
    ...
    else if (aogSettings.InclinometerInstalled == 2 && useBNO08xRoll == 0) //Add by Math: if BNO08x is use for roll, MMA is not read
    {
        if (MMAinitialized)
        {
    ...
    else if (aogSettings.InclinometerInstalled == 3 && useBNO08xRoll == 0) //Add by Math: if BNO08x is use for roll, MMA is not read
    {
        if (MMAinitialized)
        {
	...
	if (useBNO08xRoll == 0) //Add by Math: if BNO08x is use for roll, Kalman filter is not calculated
	{		
		//Kalman filter
		Pc = P + varProcess;
		G = Pc / (Pc + varRoll);
	...
	if (bno08xEnable == 1) //Add by Math: if BNO08x is enable, read BNO08x data
	{
		if (myIMU.dataAvailable() == true)
		{
			bno08xYaw = (myIMU.getYaw()) * 180.0 / PI; // Convert yaw / heading to degrees
			...
			if (aogSettings.UseMMA_X_Axis == 1) //Adafruit library: roll is rotation around X axis
			{				
				bno08xRoll = (myIMU.getRoll()) * 180.0 / PI; //Convert roll to degrees
			}
			else //Adafruit library: pitch is rotation around Y axis
			{
				bno08xRoll = (myIMU.getPitch())* 180.0 / PI; // Convert pitch to degrees
			}
			if (aogSettings.InvertRoll == 1) 
			{
				bno08xRoll *= -1.0; //Invert roll sign if needed
			}
		}
	}
  ...
  if (Serial.available() > 7 && isDataFound)
  {
    ...
    //////////////////////////////////////////////////////////////////////////////////////
      ...
      if (aogSettings.BNOInstalled && useBNO08xHeading == 0) //Add by Math: if BNO08x is used for heading, BNO055 heading is not send
      {
        Serial.print((int)IMU.euler.head);       //heading in degrees * 16 from BNO
        Serial.print(",");
      }
	  else if (useBNO08xHeading == 1) //Add by Math: if BNO08x is used, send BNO08x heading
	  {
		  float bno08xYaw_16 = bno08xYaw * 16;
		  Serial.print((int)bno08xYaw_16);
		  Serial.print(",");
	  }
      else
      ...
      if (aogSettings.InclinometerInstalled && useBNO08xRoll == 0) //Add by Math: if BNO08x is used for roll, MMA/DOGS2 roll is not send
      {
        Serial.print((int)XeRoll);          //roll in degrees * 16
        Serial.print(",");                 
      }
	  else if (useBNO08xRoll == 1) //Add by Math: if BNO08x is used for roll, send BNO08x roll
	  {
		  float bno08xRoll_16 = bno08xRoll * 16;
		  Serial.print((int)bno08xRoll_16);
		  Serial.print(",");
	  }
      else  
        Serial.print("0,");                //no Inclinometer installed
